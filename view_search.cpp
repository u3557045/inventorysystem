...
void view_search(){
  ...
  while (getline(fin,line)){
    ...
    int intprice=atoi((line.substr(n,position)).c_str());
    dynamicarray[count].price=intprice;
    n=position+1;
    position=line.find("|",n);

    int intamount=atoi((line.substr(n,position)).c_str());
  ...
  while(command!=3){
    if (command==1){
      ...
      for (int k=0;k<count;++k){
        ...
        if (filter==dynamicarray[k].id||filter==dynamicarray[k].name||filter==dynamicarray[k].category||filter==dynamicarray[k].manufacturer||filter==price||filter==amount||filter==dynamicarray[k].status)
